<div class="media">
	...
	@foreach($miServicio as $obj)
		<b>Servicio: </b>{{ date('H:i', strtotime($obj->inic_servi)) }}
		</br>
		<b>Fecha: </b>{{ date('d-m-Y', strtotime($obj->inic_servi)) }}
		</br>
		...
		<tbody id="listadoControles">
			@foreach($misProgramadas as $obj)
				<tr>
					...
					<td class="text-center">{{ date('H:i', strtotime($obj->fech_progr)) }}</td>
					@if($obj->fech_contr == "")
						<td class="text-center">--:--</td>
					@else
						<td class="text-center">{{ date('H:i', strtotime($obj->fech_contr)) }}</td>				
					@endif
